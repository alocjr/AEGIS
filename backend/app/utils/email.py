...
from app.utils.email_templates import (
    ...
    render_email_verification_text,
    render_password_reset_html,
    render_password_reset_text,
)
...
def _build_verify_link(token: str) -> str:
    base = settings.app_base_url.rstrip("/")
    return f"{base}/login?verify_token={quote(token, safe='')}"
...
def send_verification_email(to_email: str, token: str) -> bool:
    """Envia email com link de verificação. Retorna True se enviado."""
    if not smtp_configured():
        logger.warning("SMTP não configurado; email de verificação não enviado para %s", to_email)
        return False

    verify_link = _build_verify_link(token)
    expire = settings.email_verification_expire_minutes
    subject = "Confirme seu email — Valorian 4 Future"
    body_text = render_email_verification_text(verify_link, expire)
    body_html = render_email_verification_html(verify_link, expire)
